...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScriptinterpreted3InjectionForAppEvent]];
...
- (WKUserScript *)userScriptinterpreted3InjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_interpreted3 = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _interpreted3, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScriptinterpreted3JSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return url to interpreted3 in order to handle OAuth and more
